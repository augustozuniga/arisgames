...
if (isset($_REQUEST['action']) == FALSE){
	//Display the form
	echo '<h1>Database Utilities for ARIS games</h1>';
	echo "<p>You are about to modify tables for prefix '{$prefix}'</p>
			<p>If you would like to change this, edit the common.php before using this utility</p>";
...
else if ($_REQUEST['action'] == "UPDATE6731") {
	//Insert the tables that support adding applications
	$query = "ALTER TABLE  {$prefix}players ADD  timestamp TIMESTAMP ON UPDATE CURRENT_TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP";
	run_query($query);
}
